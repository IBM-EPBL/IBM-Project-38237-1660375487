...
def recommend():
    if('email' in session):
        sendGridEmail = session["email"]
        recommend_sql = "SELECT * FROM users WHERE email =?"
        recommend_stmt = ibm_db.prepare(conn, recommend_sql)
        ibm_db.bind_param(recommend_stmt,1,sendGridEmail)
        ibm_db.execute(recommend_stmt) 
        account = ibm_db.fetch_assoc(recommend_stmt)
        recipient = account["EMAIL"]
        message = Mail(
            from_email='agneslily.23ec@licet.ac.in',
            to_emails=recipient,
            subject='Job Recommendations from Job Cache',
            html_content=render_template('recommendation.html'))
        try:
